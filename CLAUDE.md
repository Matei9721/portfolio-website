...
- This is the user's personal portfolio website.
- Its purpose is to present the user's work, experience, projects, capabilities, and personality clearly and credibly.
- Decisions should strengthen the portfolio as evidence of the user's skills, not turn it into a generic demonstration site.

## Personal design and content

- The website must feel personal and specific to the user.
...
- Consult the user and obtain explicit approval before adding a new feature, section, project, claim, dependency, service, integration, tracking mechanism, asset, or significant design direction.
...
- Work may proceed without another approval only when it stays within a scope the user has already explicitly approved.

## Git rules

- Committing on the user's behalf is never allowed.
- Do not create commits, amend commits, stage files, push branches, create tags, merge branches, or open pull requests.
- Leave all changes uncommitted for the user to inspect and commit personally.
- All new work must happen on a new task-specific branch unless the user explicitly requests a worktree.
- Before editing, verify the current branch.
- If the repository is not already on the task-specific branch chosen for the work, stop and ask the user what the branch should be called and which ref it should be based on.
- Do not invent a branch name or base, and do not create or switch branches before the user answers.
